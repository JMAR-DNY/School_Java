...
public class Ball extends Thread implements Runnable {
    ...
    public void move() {
        ...
        //computer scores here
        if (location.x > rightWall )  {
            KeepBallAlive = false;//terminates while loop in run
            app.getComputerScore();
        ...
        if ( location.x < leftWall ) {
            ...
            deltaY = MINDY + (int) (Math.random() * MAXDY); // CHANGE VERTICAL DIRECTION
            location.x = leftWall;
            app.getUserScore();//user scores here
            app.informGame();//score is updated
        }
    } //END move()
